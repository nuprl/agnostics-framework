...
def main():
    while True:
        ...
        (cwd/'snippet.jl').write_text(code)
        ...
        for test_case in test_cases:
            ...
            if exit_code != 0:
                res = res_fail_error(exit_code, stderr)
                break

            if real_output.rstrip() != expected_output.rstrip():
                res = res_fail_wrong_output(expected_output, real_output, stderr)
                break

            res = res_success(stderr)
